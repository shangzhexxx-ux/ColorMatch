...
export default function ImageEditor() {
  ...
  const desktopPreviewFontPx = (() => {
    try {
      ...
      if (!card) return (isPortrait ? 13 : 15) * textScale;
      const rect = card.getBoundingClientRect();
      const w = rect.width;
      const h = rect.height;
      if (w <= 0 || h <= 0) return (isPortrait ? 13 : 15) * textScale;
      const baseFontPx = isPortrait ? 13 : 15;
      return baseFontPx * textScale;
    } catch { return (isPortrait ? 13 : 15) * textScale; }
  })();
  ...
  const reverseGeocodeCity = async (lat: number, lon: number) => {
    ...
    try {
      ...
      if (result) {
        const capitalizeFirstOfEachWord = (str: string) => {
          let upper = true;
        ...
        console.log("[ColorMatch] City result:", capitalizeFirstOfEachWord(result));
        return capitalizeFirstOfEachWord(result);
      }
      ...
      void (async () => {
        try {
          ...
          try {
            ...
            console.log("[ColorMatch] EXIF keys:", exifData ? Object.keys(exifData) : "null");
    ...
    >
      ...
      {image && (
        ...
        <div className="cm-mobile-stage lg:hidden fixed inset-0 z-[60] flex flex-col">
          <div className="flex-shrink-0 flex flex-col items-center gap-2 px-4 pt-2">
            ...
            >
                  ...
                  <div ref={mobileScrollRef} className="flex-1 overflow-y-auto overscroll-contain px-4 pb-[calc(env(safe-area-inset-bottom)+104px)]" style={{ WebkitOverflowScrolling: "touch" }}>
                    <div ref={mobileControlsRef} className="cm-controls-panel bg-[color:var(--cm-surface)] w-full max-w-[420px] mx-auto rounded-2xl border border-[color:var(--cm-border)] p-4 space-y-3">
